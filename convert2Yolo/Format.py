    ...
    def parse(csv_path, img_path):
        ...
        try:
            ...
            for line in csv_f:
                ...
                img = cv2.imread(os.path.join(img_path, "".join([filename, ".jpg"])))
                img_width = str(img.shape[0])
                img_height = str(img.shape[1])
                img_depth = 3
        ...
        try:
            with open("box_groups.txt", "w") as bboxGroups:
                ...
                for filename in filenames:
                    ...
                    img = cv2.imread(os.path.join(img_path, "".join([filename, img_type])))
                    img_width = str(img.shape[0])
                    img_height = str(img.shape[1])
                    img_depth = 3
